...
begin
	...
	string file = "attended_log_" + string(run) + ".txt";
	...
	begin
		...
		if transitioned || (k <= before_trans) then
			...
			if mod(k,trans_mod) == 0 then
				...
				if mod(trans_set,4) == 0 then
					...
					trans_set = 1;
					if trans_index == (trans_length / 2) then
						output_file switcheroo = new output_file;
						switcheroo.open("switcheroo.txt");
						switcheroo.print(k);
						switcheroo.close();
					end;
				end;
		...
		if mod(k,total_dur) == 0 then
			if is_target[letter_counter] == 1 then
				letter_index = last_n_stims[n_back];
				num_targs = num_targs + 1; 
			else
				letter_index = random_exclude(1, 26, last_n_stims);
		...
		stim_trial.present();		
		if response_manager.last_response() != last_response then
			out.print(string(k)+"\t"+string(response_manager.last_response())+"\n");
