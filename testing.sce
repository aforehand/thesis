...
array {
	LOOP $i 3;
		bitmap {filename = "\IB\\pointe_shoes_tag_$i.png";} "shoes_$i";
	ENDLOOP;
} shoes;
...
array {
	LOOP $i 3;
		bitmap {filename = "\IB\\car_tag_$i.png";} "car_$i";
	ENDLOOP;
} cars;
...
array {
	LOOP $i 3;
		bitmap {filename = "\IB\\octopus_tag_$i.png";} "octopus_$i";
	ENDLOOP;
} octopuses;
...
array {
	LOOP $i 3;
		bitmap {filename = "\IB\\helmet_tag_$i.png";} "helmet_$i";
	ENDLOOP;
} helmets;
...
#shoes-car transition
array {
	LOOP $i 20;
		bitmap {filename = "\IB\\transitions\\shoes_car_$i.png";} "shoes_car$i";
	ENDLOOP;
} shoes_cars;

#car-octopus transition
array {
	LOOP $i 20;
...
#non-target images 1
array<bitmap> non_targets_1[0][0];
non_targets_1.add(trees);
non_targets_1.add(drinks);
non_targets_1.add(cakes);
non_targets_1.add(boxes);
#non-target images 2
array<bitmap> non_targets_2[0][0];
non_targets_2.add(shoes);
non_targets_2.add(cars);
non_targets_2.add(octopuses);
non_targets_2.add(helmets);

#non-target transitions 1
array<bitmap> non_target_trans_1[0][0];
non_target_trans_1.add(trees_drinks);
non_target_trans_1.add(drinks_cakes);
non_target_trans_1.add(cakes_boxes);
#non-target transitions 2
array<bitmap> non_target_trans_2[0][0];
non_target_trans_2.add(shoes_cars);
non_target_trans_2.add(cars_octopuses);
non_target_trans_2.add(octopuses_helmets);
...
sub
	pre_trans(array<bitmap> non_targets[4][3],
		array<bitmap> non_target_trans[4][20])
begin
...
sub
	do_run (array<bitmap> nt_imgs[4][3],
		array<bitmap> nt_trans[3][20],
		array<bitmap> trans_1[trans_dur],
...
begin
	...
	begin	
		...
		elseif switcher == 0 then 
			do_the_things(b_dur, house, face, 4, images_b, images_b, i);
			stim_index = stim_index + 1;		
		#Second transition
		elseif switcher == 1 then
			do_the_things(a_dur, ho_f, f_ho, 3, trans_2[trans_set[1]], images_c, i);	
		#Second Stimulus
		elseif switcher == 2 then 
		stim_trial.set_duration(c_dur);
			do_the_things(c_dur, face, house, 4, images_c, images_c, i);
			stim_index = stim_index + 1;		
		#Third transition
		elseif switcher == 3 then
			do_the_things(a_dur, f_ho, ho_f, 3, trans_3[trans_set[1]], images_b, i);
		end;	
		onset = false;
...
begin
	...
	loop int i = 1 until i > 2 begin
		...
		if i == 1 then
			do_run(non_targets_1, non_target_trans_1, boxes_houses, houses,
				house_face_trans,	faces, face_house_trans);
		else
			do_run(non_targets_2, non_target_trans_2, helmets_faces,	faces,
				face_house_trans, houses, house_face_trans);
		end;
		i = i + 1;
	end;
	end_trial.present();
